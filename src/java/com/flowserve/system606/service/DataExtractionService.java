    ...
    public void generatePobOutput(String inputFile) throws Exception {
        ...
        try {
            ...
            PreparedStatement pst = connection.prepareStatement("INSERT INTO POB_Output "
                    ...
                    + "PROJECTED_GAIN_LOSS_BACKLOG_LC,CTD_STANDARD_COSTS_COGS_LC,LOSS_CONTRACT_ADJ_LC,SALES_DESTINATION,OEAM_DISAGG,SL_START_DATE,SL_END_DATE,COSTS_INCURRED_CTD_LC) "
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)");
            ...
            PreparedStatement contractST = connection.prepareStatement("INSERT INTO Contract_Ouput "
                    ...
                    + "LOSS_RESERVE_ADJ_CUMULATIVE_LC,GROSS_PROFIT_LOSS_PERIOD_LC,BOOKING_DATE,THIRD_PARTY_COMMISSION_TO_RECOGNIZE_CTD_LC,THIRD_PARTY_COMMISSION_TO_RECOGNIZE_PERIOD_LC,PROJECT_GAIN_LOSS_INCL_TPC_BACKLOG_LC,CONTRACT_BILLINGS_CTD_CC,TPC_TO_ACCEL_PERIOD_LC,TPC_TO_ACCEL_CTD_LC) "
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?)");
                ...
                for (String code : reportingUnits) {
                    ...
                    for (Contract contract : reportingUnit.getContracts()) {
                        for (PerformanceObligation pob : contract.getPerformanceObligations()) {
                            ...
                            pst.setBigDecimal(34, zeroIfNull(calculationService.getCurrencyMetric("LOSS_CONTRACT_ADJ_LC", pob, period).getLcValue()));
                        ...
                        contractST.setBigDecimal(9, zeroIfNull(calculationService.getCurrencyMetric("CONTRACT_LIABILITY_LC", contract, period).getLcValue()));
                        contractST.setBigDecimal(10, zeroIfNull(calculationService.getCurrencyMetric("CONTRACT_ASSET_LC", contract, period).getLcValue()));
